...
func (m *AckingResourceMutatorWrapper) WaitForFirstAck(ctx context.Context, node string, typeURL string) {
	...
	logger := m.logger.With(logfields.XDSClientNode, node, logfields.XDSTypeURL, typeURL)
	...
	if previouslyAckedVersion, exists := m.ackedVersions[nodeIP]; !exists || previouslyAckedVersion < ackVersion {
		...
		if previouslyAckedVersion < ackVersion {
			...
			if !exists || (exists && ch != nil) {
				m.logger.Info("HandleResourceVersionAck: first ACK received", logfields.XDSClientNode, nodeIP, logfields.XDSTypeURL, typeURL, logfields.XDSAckedVersion, ackVersion)
			}
